...
const monitoringTitles = [/Конотопська відьма/];
...
async function getAllEventCards(browser) {
  ...
  for (let i = 1; i <= maxPages; i++) {
    ...
    for (const card of elements) {
      const title = await card.$eval("h3.performanceCard__title", (node) =>
...
async function checkTickets(browser, eventLink) {
  ...
  const occupied = await (
    await page.getByText("Вільні місця, на жаль, закінчились")
  ).isVisible();
  const notAvailable = await (
    await page.getByText("Онлайн продаж на виставу закінчено")
  ).isVisible();

  await page.close();

  return !occupied && !notAvailable;
}
